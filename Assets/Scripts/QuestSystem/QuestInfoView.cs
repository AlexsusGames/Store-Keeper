...
public class QuestInfoView : MonoBehaviour
{
    ...
    public void SetData(QuestConfig config, int currentValue = 0)
    {
        ...
        string translatedName = Core.Localization.Translate(config.Title);
        string translatedDescribtion = Core.Localization.Translate(config.Description);

        questName.text = translatedName;
        questDescribtion.text = translatedDescribtion;
